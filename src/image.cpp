...
pe::span_t<pe::section_header> pe::image::sections() noexcept
{
...
pe::span_t<const pe::section_header> pe::image::sections() const noexcept
{
	const auto nt = nt_hdrs();

	return pe::span_t{ nt->first_section_hdr(), nt->num_sections() };
}
